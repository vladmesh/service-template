...
- Перенести логику генерации compose-блоков целиком в `sync_services`: вместо хранения шаблонов в `infra/compose.services/<slug>/base|dev.yml` скрипт должен собирать секции напрямую (например, из описателей в `services.yml` или из общих правил шаблона).
- Для каждого сервиса задокументировать стандартный compose-блок (какие env, volumes, зависимости нужны) и описать способ конфигурации в `services.yml`, чтобы `sync_services` мог синтезировать итоговый YAML.
