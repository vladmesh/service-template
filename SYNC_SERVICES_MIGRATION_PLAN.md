...
- В `Makefile` появилась цель `sync-services`: без аргументов она запускает `scripts/sync_services.py check`, а `make sync-services create` переводит скрипт в режим генерации. Аргумент также можно передать через `mode=create`.
- Запуск происходит в tooling-контейнере по тем же правилам, что и остальные make-цели, поэтому локальная среда не требуется.
- `scripts/compose_sync.py` теперь использует общие хелперы из `scripts/lib/compose_blocks.py`, которые также задействует `sync_services` — это исключает дублирование логики построения блоков.
- Документация (`AGENTS.md` и раздел Makefile cheatsheet в README) обновлена, чтобы агенты знали о новой команде.
