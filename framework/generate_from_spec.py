...
def generate_schemas(models_file: Path, output_file: Path) -> None:
    ...
    try:
        ...
        output_file.write_text(GENERATED_HEADER + content)
...
def generate_controller(rest_file: Path, models_file: Path, output_file: Path) -> None:
    ...
    # Note: We do NOT add GENERATED_HEADER to controllers
    # as they are meant to be implemented by user
...
def main() -> None:
    ...
    # 1. Global Specs (Shared)
    shared_spec_dir = project_root / "shared" / "spec"
    shared_generated_dir = project_root / "shared" / "shared" / "generated"
    ...
    print("Generating shared schemas...")
    generate_schemas(models_file, shared_generated_dir / "schemas.py")

    events_file = shared_spec_dir / "events.yaml"
    generate_events(events_file, shared_generated_dir / "events.py")
    ...
    for service_dir in services_dir.iterdir():
        ...
        print(f"Processing specs for {service_dir.name}...")

        # Generated code locations
        src_generated_dir = service_dir / "src" / "generated"
        src_controllers_dir = service_dir / "src" / "controllers"

        src_generated_dir.mkdir(parents=True, exist_ok=True)
        (src_generated_dir / "routers").mkdir(parents=True, exist_ok=True)
        src_controllers_dir.mkdir(parents=True, exist_ok=True)

        # Init files
        (src_generated_dir / "__init__.py").touch(exist_ok=True)
        (src_generated_dir / "routers" / "__init__.py").touch(exist_ok=True)

        # Generate Protocols & Routers
        print("  Generating protocols...")
        generate_protocols(spec_dir, models_file, src_generated_dir / "protocols.py")

        for router_file in spec_dir.glob("*.yaml"):
            print(f"  Processing {router_file.stem}...")

            # Generate Router
            router_output = src_generated_dir / "routers" / f"{router_file.stem}.py"
            generate_router(router_file, models_file, router_output)

            # Generate Controller Stub
            controller_output = src_controllers_dir / f"{router_file.stem}.py"
