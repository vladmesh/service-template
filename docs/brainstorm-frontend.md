...
1. **Синхронизация openapi.json в CI/CD** — `make check-openapi` по аналогии с `make check-sync`: поднять бэкенд, извлечь openapi.json, сравнить с закоммиченным. Детали реализации — в Phase 2.
2. **Кастомные (non-CRUD) операции в Refine** — `dataProvider` покрывает CRUD. Для бизнес-действий (`approve_document`, `ban_user`) — через типизированный `apiClient`, не через Refine. Разделение: Refine = CRUD-админка, apiClient = всё остальное. Детали — в Phase 4.
