...
- **Example:** `DATABASE_URL`, `API_BASE_URL` must all be explicitly configured
- **Example:** `REDIS_URL` must be explicitly configured
...
- Required for backend and any Python services that import from `shared`
...
- **Missing Broker Connection:** Publishing events without `broker.connect()` -> `AssertionError`
- **Stale Shared Code:** Forgetting volume mount -> services use old generated code
- **Type Mismatches:** Code generator now supports `list[type]` but verify complex types
- **Timezone Awareness:** Use `datetime.now(UTC)` for Pydantic `AwareDatetime` fields
- **Dockerfile Validation:** `sync_services.py` validates COPY sources - use proper Poetry patterns
</coding_guidelines>
