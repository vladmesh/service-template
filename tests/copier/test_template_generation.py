...
class TestIntegration:
    ...
    def test_migration_targets_run_alembic_in_backend_container(self, project_backend: Path):
        """Migration targets should not rely on host-side database hostnames."""
        makefile = (project_backend / "Makefile").read_text()

        assert "services/backend/.venv/bin/alembic" not in makefile
        assert (
            "$(DOCKER_COMPOSE) $(COMPOSE_DEV) run --rm --build backend alembic "
            "-c services/backend/migrations/alembic.ini upgrade head"
        ) in makefile
        assert (
            "$(DOCKER_COMPOSE) $(COMPOSE_DEV) run --rm --build backend alembic "
            '-c services/backend/migrations/alembic.ini revision --autogenerate -m "$(name)"'
        ) in makefile
